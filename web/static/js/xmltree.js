...
| ==> modified by Roberto Battistoni 2017
============== */
(function($) {
	...
	XMLTree = function(jdo, subTreeRequest) {
		...
		XMLTree.prototype.delve = function(node) {
			...
			if (!kids.length) {
				var value="<span onmouseover='XMLTree.prototype.onOver(this)'>"+node.text().trim()+"</span>";
				LITxtHolder.prepend(value).prepend(tagName);
			} else {
				//var value="<span>"+node.text()+"</span>";
				this.delve_nextAppendTo = ul;
				LITxtHolder.prepend(node.immediateText()+(!jdo.noDots ? '\n..' : '')).prepend(tagName);
	...
	//get immediate text
	//$.fn.immediateText = function() { return $(this).clone().children().remove().end().text().replace(/(\r\n|\n|\r)/gm,""); }
	$.fn.immediateText = function() { return $(this).clone().children().remove().end().text().replace(/(\r\n|\n|\r)/gm,"").trim(); }
